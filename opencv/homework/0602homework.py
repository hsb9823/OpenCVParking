...
from opencv.Common.utils import contain_pts  # 좌표로 범위 확인 함수
...
def onMouse(event, x, y, flags, param):
    ...
    if check >= 0:  # 좌표 사각형 선택 시
        pts1[check] = (x, y)
        draw_rect(np.copy(resize_img))
        warp(np.copy(resize_img))


image = cv2.imread('image/book.jpg')
resize_img = cv2.resize(image, (640, 640))
if resize_img is None: raise Exception("영상 파일을 읽기 에러")
